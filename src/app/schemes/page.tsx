    ...
    <AppShell>
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
        <motion.div variants={item}>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Government Schemes</h1>
          <p className="text-sm text-[var(--text-secondary)]">Auto-matched to your profile — find and apply for benefits</p>
        </motion.div>
        ...
        <motion.div variants={item} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search schemes..." value={search} onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[var(--border)] text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {categories.map((c) => (
              <button key={c} onClick={() => setFilter(c)}
                className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
                  filter === c ? "bg-emerald-500 text-white" : "bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-secondary)] hover:border-green-200"
                }`}>
                {c}
              </button>
            ))}
            ...
            return (
              <motion.div key={scheme.id} variants={item}
                ...
                <button onClick={() => setExpanded(isExpanded ? null : scheme.id)}
                  className="w-full p-5 flex items-start justify-between text-left hover:bg-[var(--bg-muted)]/50 transition">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-[var(--text-primary)] truncate">{scheme.name}</p>
                      <p className="text-xs text-[var(--text-secondary)] mt-0.5">{scheme.benefit}</p>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    {daysLeft !== null && (
                      <div className={`text-center ${daysLeft <= 30 ? "text-red-600" : "text-[var(--text-secondary)]"}`}>
                ...
                {isExpanded && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    className="px-5 pb-5 border-t border-[var(--border)] space-y-4">
                    <p className="text-sm text-[var(--text-secondary)] mt-3 leading-relaxed">{scheme.description}</p>
                    
                    <div>
                      <p className="text-xs font-semibold text-[var(--text-secondary)] mb-2">Required Documents</p>
                      <div className="flex flex-wrap gap-2">
                        {scheme.documents.map((doc) => (
                          <span key={doc} className="px-2.5 py-1 bg-[var(--bg-muted)] text-[var(--text-secondary)] rounded-lg text-xs font-medium flex items-center gap-1">
