                ...
                <Card padding="lg" className="space-y-5">
                      ...
                      {cropPredictionDefaults.crops.map((c) => (
                        <button key={c} onClick={() => setCrop(c)}
                          className={`p-3 rounded-xl text-sm font-medium transition-all border ${
                            crop === c ? "bg-green-50 border-green-400 text-green-700 ring-1 ring-green-200" : "bg-white border-gray-200 text-gray-600 hover:border-green-200"
                          }`}>{c}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <select value={state} onChange={(e) => setState(e.target.value)}
                      className="w-full p-3 rounded-xl border border-gray-200 text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-200 bg-white">
                      {cropPredictionDefaults.states.map((s) => (<option key={s} value={s}>{s}</option>))}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Land Size: {land} acres</label>
                    <input type="range" min={1} max={100} value={land} onChange={(e) => setLand(Number(e.target.value))} className="w-full accent-green-500" />
                    <div className="flex justify-between text-xs text-gray-400 mt-1"><span>1 acre</span><span>100 acres</span></div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {cropPredictionDefaults.soilTypes.map((s) => (
                        <button key={s} onClick={() => setSoil(s)}
                          className={`p-3 rounded-xl text-sm font-medium transition-all border ${
                            soil === s ? "bg-green-50 border-green-400 text-green-700" : "bg-white border-gray-200 text-gray-600 hover:border-green-200"
                          }`}>{s}</button>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      {cropPredictionDefaults.seasons.map((s) => (
                        <button key={s} onClick={() => setSeason(s)}
                          className={`p-3 rounded-xl text-sm font-medium transition-all border ${
                            season === s ? "bg-green-50 border-green-400 text-green-700" : "bg-white border-gray-200 text-gray-600 hover:border-green-200"
                          }`}>{s}</button>
                      ))}
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => setStep(0)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 flex items-center justify-center gap-2">
                      ...
                      return (
                        <div key={key}>
                          <div className="flex justify-between mb-1">
                            ...
                            <span className={`text-sm font-bold ${isFert ? "text-red-600" : "text-gray-900"}`}>
              ...
              <motion.div key="step3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                {loading ? (
                  <Card padding="lg" className="text-center py-16">
                    ...
                    <p className="text-lg font-semibold text-gray-700">AI is analyzing your farm data...</p>
                    <p className="text-sm text-gray-400 mt-1">Using DeepSeek Reasoner with crisis-adjusted models</p>
                  </Card>
                ) : (
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <Card className="text-center border-l-4 border-red-400"><p className="text-xs text-gray-500 font-medium uppercase">Total Cost</p><p className="text-2xl font-bold text-red-600 mt-1">{formatCurrency(result.totalCost)}</p></Card>
                      <Card className="text-center border-l-4 border-blue-400"><p className="text-xs text-gray-500 font-medium uppercase">Est. Revenue</p><p className="text-2xl font-bold text-blue-600 mt-1">{formatCurrency(result.estimatedRevenue)}</p></Card>
                      <Card className="text-center border-l-4 border-green-400"><p className="text-xs text-gray-500 font-medium uppercase">Net Profit</p><p className="text-2xl font-bold text-green-600 mt-1">{formatCurrency(result.netProfit)}</p><p className="text-xs text-green-500 mt-0.5">ROI: {result.profitabilityPct}%</p></Card>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                      <Card padding="lg">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">{result.costBreakdown.map((c, i) => (
                          <div key={c.name} className="flex items-center gap-2 text-xs"><div className="w-2.5 h-2.5 rounded-full" style={{ background: COLORS[i] }} /><span className="text-gray-600">{c.name}</span><span className="ml-auto font-semibold text-gray-800">{c.pct}%</span></div>
                        ))}</div>
                      </Card>
                      <Card padding="lg" className="space-y-4">
                        <h3 className="font-bold text-gray-900">Yield Analysis</h3>
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            ...
                            <div key={d.label} className={`${d.bg} rounded-xl p-3 text-center`}><p className="text-xs text-gray-500">{d.label}</p><p className={`text-xl font-bold ${d.color}`}>{d.value}</p></div>
                          ))}
                        </div>
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => { setStep(0); setShowResult(false); }} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium hover:bg-gray-50 flex items-center justify-center gap-2"><ChevronLeft className="w-4 h-4" /> New Prediction</button>
